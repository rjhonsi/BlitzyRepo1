...
**Notes** — Node derives `Content-Length: 34` from that literal for `GET`, and suppresses both body and `Content-Length` for `HEAD`, the application code being identical either way — which is why the body criterion is scoped to a body-bearing request. The byte-exact body is a diagnostic fingerprint of the expected response during a manual probe, not authentication and not an authenticity guarantee, and this story's benefit is read at that diagnostic scope: the body is a public compile-time literal `[server.js:9]`, and the source implements no authentication, TLS identity, signature, challenge, nonce, or secret `[server.js:1-14]`, so any process able to bind the same endpoint could reproduce the same status, header and body. Matching it tells a reader the response is the one this repository's handler produces; it does not establish which process produced it. Assumption A-07 records that a single hardcoded English response is presumed acceptable to every caller.
